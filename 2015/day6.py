...
input_data = """turn off 660,55 through 986,197
...
def instructions(grid, input):
    actions = input.split()

    if actions[0] == "turn":
        cmd = actions[1]
        x1, y1 = map(int, actions[2].split(','))
        x2, y2 = map(int, actions[4].split(','))
    elif actions[0] == "toggle":
        cmd = "toggle"
        x1, y1 = map(int, actions[1].split(','))
        x2, y2 = map(int, actions[3].split(','))
...
gridBrightness = [[0 for _ in range(1000)] for _ in range(1000)]

for i in input_data.splitlines():
    instructions(grid, i)
    brightness(gridBrightness, i)

total_on = sum(sum(row) for row in grid)
print(total_on)

total_bg = sum(sum(row) for row in gridBrightness)
print(total_bg)
